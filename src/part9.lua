...
HYPER_MODE_RATES = {}
for nature, group in pairs(HYPER_NATURE_GROUP) do HYPER_MODE_RATES[nature] = HYPER_RATE_GROUPS[group] end
...
function heartMessage(state)
  ...
  if heart == maxHeart then return "The door to its heart\nis tightly shut."
  elseif heart > maxHeart * 0.8 then return "The door to its heart\nis starting to open."
  elseif heart > maxHeart * 0.6 then return "The door to its heart\nis opening up."
  elseif heart > maxHeart * 0.4 then return "The door to its heart\nis opening wider."
  elseif heart > maxHeart * 0.2 then return "The door to its heart\nis nearly open."
  elseif heart > 0 then return "The door to its heart\nis almost fully open." end
  return "The door is about to open.\nUndo the final lock!"
end

local SCENT_ITEMS = { JOY_SCENT=true, EXCITE_SCENT=true, VIVID_SCENT=true }
...
local function installDayCareFidelity()
  ...
  if not ok or type(OverworldState) ~= "table" or OverworldState._colosseumDayCareV13 then return end
...
function shadowExperienceShare(battle, mon, split)
  if not (battle and battle.enemy and battle.enemy.def and battle.enemy.mon) then return 0 end
...
function installBattleRuntime(mod)
  ...
  BattleState._colosseumFidelityHooksV13 = { shadow=shadow, isActiveShadow=isActiveShadow, reduceHeart=reduceHeart, hyperRate=hyperRate }
  ...
  if not BattleState._colosseumFidelityInstalledV13 then
    ...
    if type(previousStartGrowIn) == "function" then
      BattleState.startGrowIn = function(self, battler, ...)
        ...
        if battler and battler.isPlayer and isActiveShadow(battler.mon) then self._colosseumPlayerAuraStartV16 = self.frame or 0 end
        return unpack(results)
      end
    end
    local previousTryRun = BattleState.tryRun
    BattleState.tryRun = function(self)
      if not BattleState._colosseumShouldCallV13(self) then return previousTryRun(self) end
      self.phase = "messages"; self.afterQueue = "menu"
      local mon = self.player and self.player.mon; local state = shadow(mon)
      self:say(Strings("%s called out to\n%s!", self.game.save.player.name, monName(self.game, mon)))
      if state and state.isShadow and state.hyperMode then
        state.hyperMode = false; local result = reduceHeart(mon, self.data, nil, "CALL")
        self:sayNext(monName(self.game, mon) .. " came to\nits senses!")
        if result and result.threshold then self:sayNext("The door to its heart\nopened a little!") end
      elseif mon and mon.status == "SLP" then mon.status=nil; self:sayNext(monName(self.game, mon).." woke up!")
      else self:sayNext("But nothing happened!") end
      self:act(function() self:executeAction(self.enemy, self.player, self:enemyAction()) end)
      self:queueResidual(self.player, self.enemy); self:act(function() self:endOfTurn() end)
    end
    ...
    BattleState.drawTextArea = function(self, ...)
      ...
      if self.phase == "menu" and not self.safari and not self.demo then
        love.graphics.setColor(1,1,1,1); love.graphics.rectangle("fill",126,126,34,10); love.graphics.setColor(0,0,0,1)
        Font.draw(BattleState._colosseumShouldCallV13(self) and "CALL" or "RUN",128,128)
      end
      if self.phase == "moveSelect" and self.player and self.player.curMoves then
        ...
        if selected and selected.id == SHADOW_MOVE then
          love.graphics.setColor(1,1,1,1); love.graphics.rectangle("fill",40,88,48,8); love.graphics.setColor(0,0,0,1); Font.draw("--/--",40,88)
        end
      end
      love.graphics.setColor(1,1,1,1); return unpack(results)
    end
    local previousUpdate = BattleState.update
    if type(previousUpdate) == "function" then
      BattleState.update = function(self, ...)
        local activeShadow = self.phase == "moveSelect" and self.player and isActiveShadow(self.player.mon)
        local input = self.game and self.game.input; local previousWasPressed = input and input.wasPressed
        if activeShadow then self.moveSwapIndex=nil end
        if activeShadow and type(previousWasPressed)=="function" then input.wasPressed=function(obj,key) if key=="select" then return false end return previousWasPressed(obj,key) end end
        local results={pcall(previousUpdate,self,...)}; if input and previousWasPressed then input.wasPressed=previousWasPressed end
        local succeeded=table.remove(results,1); if not succeeded then error(results[1],0) end; return unpack(results)
      end
    end
    local previousPerformMove = BattleState.performMove
    BattleState.performMove = function(self,user,target,moveInst,isCalled)
      local state=user and shadow(user.mon); local moveId=moveInst and moveInst.id
      if user and user.isPlayer and state and state.isShadow and state.hyperMode and moveId~=SHADOW_MOVE and not isCalled then
        local roll=self.rng(1,100)
        if roll<=45 then return previousPerformMove(self,user,target,moveInst,isCalled)
        elseif roll<=58 then
          local choices={}; for _,candidate in ipairs(user.mon.moves or {}) do if candidate.id~=moveId then choices[#choices+1]=candidate end end
          if #choices>0 then local alternate=choices[self.rng(1,#choices)]; self:sayNext(monName(self.game,user.mon).." used a different\nmove instead!"); return self:performMove(user,target,alternate,true) end
          self:sayNext(monName(self.game,user.mon).." ignored orders!"); return
        elseif roll<=68 then if not user.mon.status then user.mon.status="SLP"; self:sayNext(monName(self.game,user.mon).." took a nap\nin HYPER MODE!") else self:sayNext(monName(self.game,user.mon).." ignored orders!") end; return
        elseif roll<=78 then self:sayNext(monName(self.game,user.mon).." hurt itself\nin HYPER MODE!"); local damage=math.max(1,math.floor((user.mon.stats.hp or 1)/8)); self:applyDamage(user,damage); if user.mon.hp<=0 then self:onFaint(user) end; return
        elseif roll<=86 then self:sayNext(monName(self.game,user.mon).." tried to use its\nheld item!"); return
        elseif roll<=94 then self:sayNext(monName(self.game,user.mon).." lashed out at\nits TRAINER!"); return
        elseif roll<=98 then self:sayNext(monName(self.game,user.mon).." went back into\nits BALL!"); return end
        self:sayNext(monName(self.game,user.mon).." ignored the order!"); return
      end
      return previousPerformMove(self,user,target,moveInst,isCalled)
    end
    local previousOnFaint=BattleState.onFaint
    if type(previousOnFaint)=="function" then BattleState.onFaint=function(self,battler,...) local state=battler and shadow(battler.mon); if state then state.hyperMode=false end; return previousOnFaint(self,battler,...) end end
  end
  if not ItemEffects._colosseumFidelityInstalledV13 then
    ...
    ItemEffects.use=function(data,save,id,target,battle,...)
      ...
      if state and state.isShadow then
        if battle and state.hyperMode and not SCENT_ITEMS[id] then return "failed", { monName({data=data},target).." won't accept items\nin HYPER MODE!" } end
        if isRestrictedShadowItem(id) then return "failed", { "The door to its heart\nblocks that item." } end
      end
